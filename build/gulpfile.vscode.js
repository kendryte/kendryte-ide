...
gulp.task('minify-vscode', ['clean-minified-vscode', 'optimize-index-js'], common.minifyTask('out-vscode', `${sourceMappingURLBase}/core`.slice(0, 0)));
...
function packageTask(platform, arch, opts) {
	...
	return () => {
		...
		const deps = gulp.src(depsSrc, { base: '.', dot: true })
			...
			.pipe(createAsar(path.join(process.cwd(), 'node_modules'), ['**/*.node', '**/*.exe', '**/*.dll', '**/vscode-ripgrep/bin/*', '**/node-pty/build/Release/*'], 'app/node_modules.asar'));
