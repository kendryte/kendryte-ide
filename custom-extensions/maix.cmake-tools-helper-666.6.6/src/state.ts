...
export class CMakeToolsHelper {
	private _ext: vscode.Extension<any>;

	get cmakeTools() {
		if (!this._ext) {
			this._ext = vscode.extensions.getExtension('maix.cmake-tools');
		}
		return this._ext;
	}
	...
	constructor() {
		...
		helper.installAtLeaseOneCmake().then(async (installedCMakePath) => {
			await vscode.workspace.getConfiguration('cmake').update('cmakePath', installedCMakePath, true);
			const version = helper.computeCMakeVersion(installedCMakePath);
			const useCMakeServer = compare_versions(version, '3.7.1') > 0;
			await vscode.workspace.getConfiguration('cmake').update('useCMakeServer', useCMakeServer, true);
		}, (e) => {
			console.error('failed to install at least one cmake: %s', e.stack);
			vscode.window.showErrorMessage('can not download cmake, please check your network: ' + e.message);
		});
