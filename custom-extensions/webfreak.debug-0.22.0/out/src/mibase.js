...
class MI2DebugSession extends vscode_debugadapter_1.DebugSession {
    ...
    handleBreakpoint(info) {
        let event = new vscode_debugadapter_1.StoppedEvent("breakpoint", parseInt(info.record("thread-id")));
        event.body.allThreadsStopped = info.record("stopped-threads") == "all";
        this.sendEvent(event);
    }
    handleBreak(info) {
        let event = new vscode_debugadapter_1.StoppedEvent("step", parseInt(info.record("thread-id")));
        event.body.allThreadsStopped = info.record("stopped-threads") == "all";
        this.sendEvent(event);
    }
    handlePause(info) {
        let event = new vscode_debugadapter_1.StoppedEvent("user request", parseInt(info.record("thread-id")));
        event.body.allThreadsStopped = info.record("stopped-threads") == "all";
        this.sendEvent(event);
    }
    stopEvent(info) {
        if (!this.started)
            this.crashed = true;
        if (!this.quit) {
            let event = new vscode_debugadapter_1.StoppedEvent("exception", parseInt(info.record("thread-id")));
            event.body.allThreadsStopped = info.record("stopped-threads") == "all";
            this.sendEvent(event);
        }
    }
