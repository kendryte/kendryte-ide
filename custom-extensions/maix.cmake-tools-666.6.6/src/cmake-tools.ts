...
export class CMakeTools implements vscode.Disposable, api.CMakeToolsAPI {
    ...
    ready.then(() => {
      ...
      websock_server.on('connection', client => {
        ...
        client.onmessage = msg => {
          ...
          this._handleCacheEditorMessage(data.method, data.params)
              .then(ret => {
  ...
  private async _startNewCMakeDriver(cmake: CMakeExecutable): Promise<CMakeDriver> {
    ...
    drv.onProjectNameChanged(name => {
      this._statusBar.setProjectName(name);
    });
   ...
   */
  selectKit() {
    return this._kitManager.selectKit();
  }
  ...
  private async _doConfigure(cb: (consumer: diags.CMakeOutputConsumer) => Promise<number>): Promise<number> {
    ...
    if (!this._kitManager.hasActiveKit) {
      log.warning('No kit selected yet. Asking for a Kit first.');
  ...
  async build(target_?: string): Promise<number> {
    ...
    try {
      ...
      consumer.onProgress(pr => {
        this._statusBar.setProgress(pr.value);
      });
      log.showChannel();
  ...
  async debugTarget(): Promise<vscode.DebugSession|null> {
    ...
    vscode.commands.executeCommand('workbench.view.debug'); // switch to debug panel
    // const user_config = this.workspaceContext.config.debugConfig;

    if (!this._kitManager.activeKit.compilers) {
      vscode.window.showErrorMessage('gdb not found in toolchain binary folder.');
      return null;
    }
  ...
  public async quickStart(): Promise<Number> {
    ...
    await fs.writeFile(mainListFile, `
cmake_minimum_required(VERSION 3.0.0)

project(${projectName})
add_source_files(*.c *.h *.cpp *.hpp *.C *.H *.CPP *.HPP)
...
`);
  ...
  get diagnostics() {
    return Promise.resolve(this._configureDiagnostics);
  }
