...
Please install or configure a preferred generator, or update settings.json or your Kit configuration.`);
  ...
  private async getSdkPath() {
    if (!this._sdkPathCache) {
      const rootProd = await findItemParents('SDK');
      const rootDev = await findItemParents('maix-sdk');
      if (!rootDev && !rootProd) {
      ...
      if (rootProd) {
        ...
        if (!await fs.exists(`${ rootDev}/build/archive/`)) {
          throw new Error(`Maix SDK dev mode was found, but not compiled. Please run "distribuite.sh" in SDK's root dir.`);
        }
        this._sdkPathCache = {
          production: false,
          root: rootDev,
          PREPEND: `include(${ rootDev}/build/archive/cmake/common.cmake)\n`,
          APPEND: `include(${ rootDev}/build/archive/cmake/executable.cmake)\n`,
        };
      }
    }
    return this._sdkPathCache as {production: boolean; root: string; PREPEND: string; APPEND: string};
  }
}

async function findItemParents(item: string): Promise<string> {
  let pr = '';
  const paths = process.argv0.split(sep).map((partLR: string) => {
    pr += partLR + '/';
    return `${pr}${item}`;
  });
