...
const systemFindPaths: string[] = [];

vscode.workspace.onDidChangeWorkspaceFolders(updateSystemFindPaths);
updateSystemFindPaths();

function updateSystemFindPaths() {
  systemFindPaths.length = 0;
  const parentPaths: string[] = [];
  /*
    if (vscode.workspace.workspaceFolders) {
      const rootPath = vscode.workspace.workspaceFolders[0];
      let pc = '';
      rootPath.uri.fsPath.split(sep).forEach((partLR: string) => {
        pc += partLR + '/';
        parentPaths.unshift(pc);
      });
    }
  */
  let pr = '';
  process.argv0.split(sep).forEach((partLR: string) => {
    pr += partLR + '/';
    parentPaths.unshift(pr);
  });

  parentPaths.forEach((item: string) => {
    systemFindPaths.push(item + 'maix-toolchain/dist/bin');
    systemFindPaths.push(item + 'toolchain/bin');
  });
}

function convertMingwDirsToSearchPaths(mingwDirs: string[]): string[] {
  return mingwDirs.map(mingwDir => path.join(mingwDir.replace(/\/bin\/?$/, ''), 'bin')).concat(systemFindPaths);
}
...
export class KitManager implements vscode.Disposable {
  ...
  async selectKit(): Promise<Kit | null> {
    ...
    if (this._kits.length === 0) {
      ...
      switch (chosen.action) {
        case 'openConfig': {
          await vscode.commands.executeCommand('workbench.action.showMaixSettings');
          return null;
        }
        case 'cancel': {
          return null;
  ...
  async rescanForKits() {
    ...
    const discovered_kits = await scanForKits(convertMingwDirsToSearchPaths(this.config.toolchainSearchDirs));
  ...
  private async _rereadKits() {
    ...
    for (const kit_path of [this._userKitsPath, this._projectKitsPath]) {
      ...
      for (const kit of await readKitsFile(kit_path)) {
        if (!kit.compilers) {
          continue;
        }
