...
import { ISerialMonitorService, ISerialPortInstance } from 'vs/kendryte/vs/workbench/serialPort/terminal/common/terminal';
...
export class TerminalEntry extends QuickOpenEntry {
	...
	constructor(
		public instance: ISerialPortInstance,
		private label: string,
		private terminalService: ISerialMonitorService
	) {
		super();
	}
...
export class CreateTerminal extends QuickOpenEntry {
	...
	public run(mode: Mode, context: IEntryRunContext): boolean {
		if (mode === Mode.OPEN) {
			setTimeout(() => this.commandService.executeCommand('workbench.action.kendryte.terminal.new'), 0);
			return true;
		}
...
export class TerminalPickerHandler extends QuickOpenHandler {
	...
	public getResults(searchValue: string): TPromise<QuickOpenModel> {
		...
		terminalEntries.push(new CreateTerminal(nls.localize("workbench.action.kendryte.terminal.newplus", "$(plus) Create New Integrated Terminal"), this.commandService));
